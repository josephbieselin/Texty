...
#define REP_MAN_DIR		"/var/www/html/2"		// directory path for this replication manager
#define FILES_DIR 		"/var/www/html/2/files"	// directory path for files
#define ALL_USERS_TXT	"/var/www/html/2/files/all_users.txt"	// filepath for the master "all_users.txt" file
...
void handle_php_args(const string php_args, int connfd)
{
	...
	// will contain data that PHP will interpret on client-side as to if the call was successful or not
	string return_str;
	...
	*/
	...
	if (func_to_call == REGISTER_STR) {
		all_user_file.use_file(func_to_call, un, email, pw, fn, ln, return_str, 1); // return_str will contain username,email,firstname,lastname if successful
		...
		all_user_file.use_file(func_to_call, un, email, pw, garbage, garbage, return_str, 1);
		...
		all_user_file.use_file(func_to_call, un, email, pw, garbage, garbage, return_str, 0);

		replicate_data = false; // login is a read operation so we do not need to replicate a read in other RMs
	}

	else if (func_to_call == SEARCH_STR) {
		all_user_file.use_file(func_to_call, un, email, garbage, garbage, garbage, return_str, 0);
	...
	if (send_data) {
		...
		if ( return_message_size != write(connfd, return_str.c_str(), return_str.size()) ) {
			perror("write to connection failed for RM 2\n");
		}
	}
	...
	if (replicate_data) {
		...
		*/
		...
		int sockfd; // listenfd;
		struct addrinfo hints, *rm_addr; // socket address info structs

		// zero out the addrinfo struct for hints
		memset(&hints, 0, sizeof(hints));

		// specify the socket info to be used
		...
		for (size_t i = START_PORT; i <= END_PORT; ++i) {
			...
			if (i != THIS_PORT) {
				// put the address info into rm_addr, hints is not needed
				getaddrinfo(HOST_NAME, to_string(i).c_str(), &hints, &rm_addr);

				while ((sockfd = socket(rm_addr->ai_family, rm_addr->ai_socktype, rm_addr->ai_protocol)) == -1) {
					// loop until a socket is created
					cerr << "Unable to create a socket for data replication.\n";
				}

				if (connect(sockfd, rm_addr->ai_addr, rm_addr->ai_addrlen) == -1) {
...
int main(int argc, char **argv) {
    ...
    if ((listenfd = socket(AF_INET, SOCK_STREAM, 0)) == -1) {
        perror("Unable to create a socket for RM 2");
        exit(1);
    }
    ...
    servaddr.sin_port        = htons(THIS_PORT);

    // 3. "Bind" that address object to our listening file descriptor
    if (bind(listenfd, (SA *) &servaddr, sizeof(servaddr)) == -1) {
        perror("Unable to bind port for RM 2");
        exit(2);
    }

    // 4. Tell the system that we are going to use this sockect for listening and request a queue length
    if (listen(listenfd, LISTENQ) == -1) {
        perror("Unable to listen for RM 2");
        exit(3);
    }
    ...
    for ( ; ; ) {
		...
		fprintf(stderr, "Ready to connect for RM 2.\n");
	        if ((connfd = accept(listenfd, (SA *) NULL, NULL)) == -1) {
	            perror("accept failed for RM 2");
	            exit(4);
		}
		fprintf(stderr, "Connected for RM 2\n");

        // We now have a connection.  Do whatever our task is.

		/* ---------------------------- HANDLING MESSAGE PASSING OVER NETWORK -------------------------------------*/
		char php_args[PHP_MSG_SIZE];	// first 5 bytes will be the function to call in C++; remaining message will be passed in user info
		int read_bytes = read(connfd, php_args, PHP_MSG_SIZE); // number of bytes read from connection file descriptor
		// assign the first PHP_MSG_SIZE chars of php_args to the stringified version str_php_args
		string str_php_args;
		str_php_args.assign(php_args, PHP_MSG_SIZE);
		...
		if (read_bytes == PHP_MSG_SIZE) {
			...
			lk.lock();
			threads.push_back( thread( [str_php_args, connfd] { handle_php_args(str_php_args, connfd); } ) );
			lk.unlock();
			// handle_php_args also closes the connfd once completed

		} else { // the connection did not read as many bytes as was passed
			perror("read from connection failed for RM 2");
		}
    }
