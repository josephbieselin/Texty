	...
	*/
	...
	if (send_data) {
		...
		if ( return_message_size != write(connfd, return_str.c_str(), return_str.size()) ) {
			perror("write to connection failed for RM 3\n");
		}
	}
...
int main(int argc, char **argv) {
    ...
    if ((listenfd = socket(AF_INET, SOCK_STREAM, 0)) == -1) {
        perror("Unable to create a socket for RM 3");
        exit(1);
    }
    ...
    for ( ; ; ) {
		...
		fprintf(stderr, "Ready to connect for RM 3.\n");
	        if ((connfd = accept(listenfd, (SA *) NULL, NULL)) == -1) {
	            perror("accept failed for RM 3");
	            exit(4);
		}
		fprintf(stderr, "Connected for RM 3\n");
		...
		if (read_bytes == PHP_MSG_SIZE) {
			...
			perror("read from connection failed for RM 3");
		}
    }
